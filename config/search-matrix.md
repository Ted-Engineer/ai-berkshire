...
全程搜索**合计≥120次**（每次调用追加记录到 `.claude/.workflow/search-log.txt`）。
构成：GICS 25组×2视角=50 + AI赛道17×2=34 + 非AI主题≥8 + 7维补充 + 来源H≥20 + 来源I≥20（AI修复5+加密修复15） ≈ 132+，留冗余。
...
| E | 行业分布缺口反向映射 | 按分布偏差反向映射到GICS行业清单，对缺口行业定向搜索 | 分布偏差被定向修正 |
...
| **H** | **爆发股猎手（必选）** | 用ddg-search MCP执行4轮×≥5组关键词全网扫描（固定max_results=20、region=us-en）；DNA 5+1画像过滤（收入≥100%/盈利拐点/市值$3-80B/赛道纯正/积压可见 + 覆盖错位） | 传统价值筛选器系统性过滤掉三位数增速股 |
...
**来源H执行证据**：必须展示"4轮搜索词原文→每轮返回结果数→入池ticker清单→DNA验证表"。
**来源I执行证据**：必须展示"搜索词→候选→估值修复DNA 4条验证表（好生意/估值极端/恐惧归因/催化剂）"。
