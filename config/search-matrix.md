...
| **H** | **爆发股猎手（必选）** | 用open-websearch MCP执行4轮×≥5组关键词全网扫描，寻找“NBIS@$70”级早期爆发股；6条DNA画像过滤（收入≥100%/盈利拐点/市值$3-50B/赛道纯正/积压可见/覆盖不足） | 传统价值筛选器系统性过滤掉三位数增速股（NBIS教训：所有搜索词带undervalued→454%增速股被漏掉）；必须用MCP而非内置WebSearch以确保覆盖度 |

**来源F执行证据**：必须展示“Read了哪些memory文件→提取了哪些ticker”。
**来源G执行证据**：必须展示“对哪些持仓搜索了供应商/客户”。
**来源H执行证据**：必须展示“4轮搜索词原文→每轮返回结果数→入池ticker清单→6条DNA验证表（每只候选×6条=✅/❌）”。
