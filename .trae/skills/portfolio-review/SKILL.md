...
This skill is generated from `skills/portfolio-review.md` so Claude Code, Codex, and TRAE users share one canonical workflow.
...
将所有持仓按"预期年化回报"排序（v2 修订：**必须含动量列**——9/3 教训：纯期望排序在动量段系统性看空强势股，CRCL/MSTR 排最后但 3 日 +21%/+11%）：

| 排名 | 标的 | 当前占比 | 预期年化回报 | 确定性 | 预期×确定性 | 30日动量* |
|:----:|------|:-------:|:----------:|:------:|:----------:|:--------:|
| 1 | | | | | | |

*30日动量 = 当前价 / 30 日前收盘价 - 1（Yahoo Finance 1mo 数据自动拉取）。

**动量解读规则**（不改变排序，但必须呈现）：
- 动量 >+20% 的标的：标注 🔥，并注明"处于事件/动量段——期望排序对此类标的失真，是否持有应参考 catalysts.md 事件判据而非三情景期望"
- 动量 <-15% 的标的：标注 ❄️，注明"论文验证压力期——检查证伪线是否触发"
- **排名最后但带 🔥 的持仓不得仅因排序建议卖出**——需引用该标的的事件判据（config/catalysts.md）作为卖出依据
...
**关键问题**：排名最后的持仓，预期回报是否高于现金（无风险利率~4%）？如果不是，应该卖出换成现金——**除非该持仓带 🔥 且其 catalysts.md 事件判据未触发**。

**偏差裁决升级（v2，适用于加密/单一主题暴露）**：实际暴露超温度带上限且该仓位 30 日动量 >+20% 时，不自动输出"减仓修正"结论，改为呈现三选项（按带修正/切换 aggressive 档/维持现状）交用户裁决——规则详见 `config/portfolio-targets.md` v5.5。
