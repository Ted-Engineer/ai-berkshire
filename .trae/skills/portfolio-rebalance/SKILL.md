...
This skill is generated from `skills/portfolio-rebalance.md` so Claude Code, Codex, and TRAE users share one canonical workflow.
...
- Use shared project tools from `tools/` in this repository. Prefer running commands from the repository root with paths like `python tools/financial_rigor.py ...`.
- Before starting research, run the `date` command to confirm today's date; treat it as the baseline for "latest" data and state the data cutoff date in the report header. Never assume the current date from training data.
- Preserve the research quality rules from `AGENTS.md`: cross-check financial data, use exact arithmetic tools for valuation/math, and clearly label uncertainty and source gaps.
- Report output follows the same naming convention: `reports/{company}/` or `reports/{company}-{type}-{YYYYMMDD}.md`.
...
3. **磁盘状态模式**（本流程长，上下文会被压缩——一切中间产物即时落盘，中断后重开会话凭磁盘状态续跑）：
   ...
   - 搜索：每次调用后手动追加一行 `UTC时间 | 工具名 | 搜索词` 到 `.claude/.workflow/search-log.txt`，并写对应 `.used` 标记（文件名 = 实际所用搜索工具名 + ".used"，如 mcp-ddg-search.used / builtin-websearch.used）
...
5. 环境适配：Windows下用 `python`（无python3）；持仓研究用 Agent 工具**同一条消息多个调用并发**（subagent_type: general-purpose）；**搜索工具无固定优先级与回退链，由 agent 根据当前运行环境可用能力自行选择**（内置 WebSearch / 各 MCP 搜索工具 / Bash curl 直连等均可；`.claude/.workflow/SEARCH-TOOLKIT.md` 仅作命令示例参考）。禁止以任何工具不可用为由跳过搜索步骤、禁止用训练知识冒充联网结果。
6. **搜索正文获取铁律（v5.4新增，不可违反）**：使用任何搜索工具时，搜索结果（标题+snippet）**不足以作为分析依据**。必须对关键结果调用当前环境可用的正文获取方式（WebFetch / fetch_content / curl 等，工具按环境自选）获取正文全文，然后才能用作数据点。规则：
...
- 判断标准：✅“调用/investment-checklist BR，Agent ID: xxx，六关全过”=已执行；❌“我用WebSearch搜了BR财务数据”=未执行；❌“搜索工具不可用所以跳过”=未执行（必须换其他可用搜索方式）；❌“1个Agent分析3只股票”=未执行；❌“1个Agent四维度合并分析”=未执行
...
- **延时交易异动（必须）**：用当前环境可用的行情 API（如 Yahoo v8 chart API `includePrePost=true`，或腾讯 qt.gtimg.cn 等可直连源）获取全部持仓的夜盘/盘前最后成交价，与收盘价对比；延时变动>±1%的标的立即标注为异动信号，纳入优先评估
...
- **加密方向细分档判定（v5.5 修订并入 v6.0，必查）**：加密热度分之外，确认细分档位（🟢积累/🟡复苏/**🟡+复苏加速**/🟠狂热/🔴崩盘）。**🟡+ 判定**：F&G 60-85 且自 <30 回升 + BTC 底部回升 20-60% + ETF 月净流入 >$2B + 空头清算 >$1B + 事件催化 <30 天——满足 ≥3 条即判 🟡+。**9/2 教训**：缺此带导致复苏-狂热过渡段（CRCL 20 日 +57%）被误判为"超上限必须减仓"。输出：加密细分档 + 30 日动量数据行（BTC 与各加密持仓）
- **加密风险偏好档（每次执行开头必须向用户确认或沿用上次声明）**：standard / aggressive——aggressive 取各方向热度映射额度上限运行并接受更深回撤
- **偏差裁决升级条款（v5.5，优先级高于额度偏离修正）**：任一方向实际暴露 > 热度映射上限 且 该仓位 30 日动量 >+20% → 不自动触发🔴强制减仓，报告原话呈现三选项（①按映射修正②切换 aggressive 档③维持现状接受标注）交用户裁决；用户裁决后 48h 内不重复追问，daily-brief 保留一行标注
...
- **延时价格口径**：组合市值与分布计算必须使用最新可得价格（优先级：盘前 > 夜盘 > 收盘价）；报告持仓表必须同时展示「收盘价」「夜盘/盘前价」「延时vs收盘」三列；延时价格获取方法由 agent 按环境自选可用行情 API（示例：Yahoo v8 chart `interval=1m&range=1d&includePrePost=true`，取 timestamp+close 序列最后一个有效成交，判断时段：≥16:00 ET=夜盘、<9:30 ET=盘前；大陆直连可用腾讯 qt.gtimg.cn）
...
**最高优先级原则：质量 > 速度。这是投资，不是赶作业。准确、全面的分析比省时间和token重要一万倍。任何以"效率""节省""时间"为理由的合并行为都是对用户资金的不负责任。**
...
- ❌ 用搜索工具直接分析替代4-Agent并行研究
- ❌ 以"配额不足""效率考虑""节省时间""减少token"为由减少Agent数量
- ❌ 以"本会话已分析过"为由跳过任何一只
- ❌ **第2只、第3只...第N只股票降级为单Agent**（这是最常见的偷懒模式——第1只正常执行4Agent，后续“觉得太慢”而退化。绝对不允许。）

**反退化强化条款（v5.4新增，针对“第2只开始偷懒”模式）**：
- 每只股票完成后，必须在报告中打印：“✅ {TICKER}: 4个独立Agent已执行，ID: xxx/yyy/zzz/www”
- 如果任何一只股票的Agent数量<4，该股票分析**自动无效**，必须立即重做
- **“美其名曰节省时间”= 对用户资金的不负责任**。用户明确声明：不需要省时间和token，需要的是准确和全面
- 第10只股票和第1只股票的分析质量必须完全一致，不允许任何退化
...
**扫描工具**：用当前环境可用的搜索工具执行全网覆盖式搜索（无强制工具，agent 按环境自选）；每轮搜索≥10组不同关键词，每组返回结果逐条审查。
...
2. 对每只候选用当前环境可用的行情 API（如 Yahoo v8 / 腾讯 qt.gtimg.cn 等可直连源）获取现价/市值，过滤市值不在$3-80B区间的
...
**搜索执行**：按 `config/search-matrix.md` 的词库——GICS 25组全部×2视角（≥50次）、AI 17赛道全部×≥2视角（≥34次）、非AI 14主题选≥8（主题13/14优先）、7维各≥1次（D6/D7必做）、**来源H爆发股猎手≥20次**、**来源I估值修复猎手≥20次（AI修复≥5+加密修复≥15）**；合计≥120次；AI候选≤65%；每次搜索后追加search-log.txt。
...
**绝对禁止**：以"之前已执行过漏斗/瓶颈扫描"为由跳过；只搜2-3个行业就声称全市场扫描完成；跳过来源A/F/G/H；所有搜索词全部带"undervalued"。
...
| 第三步后 | bash scripts/workflow-gate-hook.sh | +checklist/funnel/bottleneck .done、candidates≥300、搜索≥110（含H≥20、I≥20）、used标记存在（任一所用搜索工具的.used） |
