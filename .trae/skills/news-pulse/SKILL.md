---
name: news-pulse
description: "公司新闻脉搏：股价异动时快速归因，用4个并行Agent侦察事件/政策/对手/情绪。当持仓股出现异常涨跌（单日±5%或一周±10%），需要快速搞清楚发生了什么时调用。"
---

## TRAE adapter note

This skill is generated from `skills/news-pulse.md` so Claude Code, Codex, and TRAE users share one canonical workflow.
...
- **description**:
  ...
  4. **大 V 观点**：可调用 `python tools/xueqiu_scraper.py` 抓段永平等大 V 最近相关发言
     ...
     - 命令示例：`python tools/xueqiu_scraper.py --user-id 1247347556 --keywords {公司名},{股票代码} --output /tmp/dyp-{公司名}.md`
