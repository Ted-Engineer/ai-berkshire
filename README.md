...
[实盘业绩](#real-track-record) · [为什么不能直接问AI](#为什么不能直接问-ai) · [Skills 一览](#skills-一览22个) · [快速开始](#快速开始) · [实战报告](#实战研究报告) · [设计理念](#设计理念) · [公众号](#精选研究首发于公众号)
...
| [`/portfolio-rebalance`](skills/portfolio-rebalance.md) | 组合调仓全流程 | 全量调仓流水线：持仓重研 + 全市场候选扫描(≥300只) + 双重准入验证 + 温度判定，产出操作信号 |
| [`/daily-brief`](skills/daily-brief.md) | 每日投研简报 | 行情+新闻+组合监控+触发价扫描，主线程快速完成，并路由到对应 skill |
...
除以上 22 个 skill 外，Claude Code 自带一个 `/deep-research` 深度研究编排器（内置于客户端，不由本仓库分发，安装 Claude Code 即可使用）。它的流程是：把问题拆成 5 个检索角度并行搜索 → 抓取来源、提取可证伪论断 → 每条论断由 3 个独立 Agent 对抗验证（3 票中 2 票证伪才剔除）→ 按置信度合成带引用来源的报告。核心价值是每条结论都被人试图推翻过，而不是搜到什么写什么。
