...
let roundId = 0;
function logRound(r) {
  rounds.push({ round: ++roundId, ts: Date.now(), ...r });
}
...
const manualAdds = ["GOOGL", "GOOG", "META", "NFLX"].map((s) => ({
  symbol: s,
  name: "manual-add communication services",
  lastsale: "",
  netchange: "",
  pctchange: "",
  marketCap: "",
...
fs.writeFileSync(
  DIR + "/data/universe-tech.json",
  JSON.stringify(
    {
      asof: "2026-08-07 close",
      source: "api.nasdaq.com + manual comm-services adds",
...
for (const row of universe) {
  ...
  const isCommon =
    !/\b(Right|Warrant|Unit|Rights)\b/i.test(name) && !sym.includes("."); // dots = classes/rights on nasdaq
  ...
  logRound({
    type: "screen",
    ticker: sym,
    mcap_b: mcap ? +(mcap / 1e9).toFixed(2) : null,
    checks: { isCommon, bigEnough, priceOk },
    pass,
  });
  if (pass)
    screened.push({
...
function parseFinvizTable(html) {
  ...
  const headers = thead
    ? [...thead[1].matchAll(/<th[^>]*>([\s\S]*?)<\/th>/g)].map((m) =>
        m[1]
  ...
  for (const rm of tbody.matchAll(/<tr[^>]*>([\s\S]*?)<\/tr>/g)) {
    const cells = [...rm[1].matchAll(/<td[^>]*>([\s\S]*?)<\/td>/g)].map((m) =>
      m[1].replace(/<[^>]+>/g, "").trim(),
    );
...
async function pullAll(view, sector, fieldsMap) {
  ...
  while (offset < 1200 && pages < 60) {
    ...
    for (const cells of rows) {
      ...
      for (const [h, key] of fieldsMap) {
        if (rec[h] !== undefined)
          logRound({ type: "metric", ticker: t, field: key, value: rec[h] });
      }
    }
...
const valMap = [
  ...
  ["EPS Past 5Y", "eps_5y_pct"],
  ["EPS Next 5Y", "eps_next5y_pct"],
  ["Sales Past 5Y", "sales_5y_pct"],
  ["Price", "price"],
  ["Change", "chg"],
];
const perfMap = [
...
console.log(
  "finviz tech valuation pages:",
  await pullAll(121, "sec_technology", valMap),
);
console.log(
  "finviz comm valuation pages:",
  await pullAll(121, "sec_communication_services", valMap),
);
console.log(
  "finviz tech performance pages:",
  await pullAll(141, "sec_technology", perfMap),
);
console.log(
  "finviz comm performance pages:",
  await pullAll(141, "sec_communication_services", perfMap),
);
...
console.log(
  ...
  merged.length,
  "with finviz data:",
  merged.filter((m) => m.fv && m.fv.pe !== undefined).length,
);
