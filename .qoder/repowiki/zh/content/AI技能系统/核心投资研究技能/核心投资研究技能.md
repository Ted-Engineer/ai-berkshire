...
## 更新摘要
**变更内容**   
- **组合调仓技能重大增强**：Source I加密修复搜索矩阵从5组扩展到15组关键词，覆盖交易所、稳定币、矿企、BTC代理、支付系统、DeFi协议、RWA代币、区块链基础设施、托管解决方案、ETF、监管发展、硬件制造商、区块链游戏平台等全面加密类别
- **标准化源分类**：统一A-I来源的分类标准 across all sources
- **八类验证检查**：新增八-category validation checks确保候选质量
- **增强集中度检查**：改进的concentration checks防止过度集中风险
- **提高搜索频率要求**：最低搜索次数从80次提升至≥120次
- **DNA评分标准更新**：采用5+1概率方法替代严格的6条件AND门，更符合实际发现场景
- **统一门禁验证逻辑**：标准化的gate validation logic提升流程一致性
...
## 简介
本文件面向投资研究与资产管理从业者，系统化梳理并文档化仓库中的"核心投资研究技能"，覆盖企业深度研究、财报审阅、行业分析、投资组合审查与财务数据分析五大关键能力。**v5.4版本通过组合调仓技能的重大增强和搜索方法论强化**，显著提升了研究数据的准确性和可靠性。文档聚焦每个技能的功能特性、参数说明、输入输出格式、使用示例与最佳实践，并提供技能协作模式与组合工作流，帮助读者快速落地高质量投研流程。
...
PB["portfolio-rebalance/SKILL.md"]
end
subgraph "团队编排"
...
- 功能要点
  - 全流程调仓研究与操作信号生成，支持全量、轻量与子集三种模式。**v5.4版本进行了重大增强**。
- 典型参数
...
**v5.4重大增强**
- **扩展搜索矩阵加密**：Source I从5组扩展到15组关键词，全面覆盖加密资产生态
- **标准化源分类**：统一A-I来源的分类标准
- **八类验证检查**：新增八-category validation checks
- **增强集中度检查**：改进concentration checks
- **提高搜索频率**：最低搜索次数从80次提升至≥120次
- **DNA评分更新**：采用5+1概率方法替代严格6条件AND门
- **统一门禁逻辑**：标准化gate validation logic

章节来源
- [codex-skills/portfolio-rebalance/SKILL.md](file://codex-skills/portfolio-rebalance/SKILL.md)

## 架构总览
以下图示展示六大核心技能与团队编排、安装同步的关系，以及常见数据流向。**v5.4版本强化了搜索方法论和工作流门禁机制**，特别是组合调仓技能的加密资产搜索能力得到大幅增强。
...
- 职责边界
  - 执行全流程调仓研究，支持多种模式和严格的准入验证。**v5.4版本进行了重大增强**。
- 关键流程
...
### 搜索正文获取铁律（v5.4新增）
- 功能要点
  - **强制正文验证**：搜索结果（标题+snippet）不足以作为分析依据，必须获取网页正文全文。
- 核心规则
  - **验证阶段必须获取正文**：用于估值/增速/合同等关键数据点的搜索结果，必须调用fetchWebContent或WebFetch获取正文确认具体数字。
  - **发现阶段可简化**：仅用于"发现候选ticker"的搜索可以只看标题。
  - **禁止snippet分析**：禁止仅凭snippet中的片段数字就写入报告。
- 执行工具
  - **open-websearch MCP**：使用`fetchWebContent`获取网页正文。
  - **内置WebFetch**：作为备用方案获取网页内容。
  - **Bash curl工具**：当MCP不可用时，使用curl直连抓取网页内容。
- 质量控制
  - 每个关键数据点都必须有正文验证记录。
  - 搜索日志中必须记录fetchWebContent调用。
  - 工作流门禁会检查搜索工具的正常使用。

**章节来源**
- [codex-skills/portfolio-rebalance/SKILL.md:41-45](file://codex-skills/portfolio-rebalance/SKILL.md#L41-L45)
- [.claude/.workflow/SEARCH-TOOLKIT.md:27-31](file://.claude/.workflow/SEARCH-TOOLKIT.md#L27-L31)
...
- 组合调仓工作流（v5.4增强版）
  - 投资组合审查发现风险集中 → 启动组合调仓 → 执行九路候选发现 → 强制正文验证 → 四大师并行分析 → 双重验证 → 输出调仓方案。
...
## 结论
六大核心技能构成完整的投资研究闭环：**v5.4版本通过组合调仓技能的重大增强和搜索方法论强化显著提升了研究质量和流程可靠性**。特别是Source I加密搜索矩阵从5组扩展到15组，DNA评分标准从严格6条件AND门改为更灵活的5+1概率方法，搜索频率要求从80次提升至≥120次，这些增强使得系统能够更全面地捕捉加密资产投资机会，同时保持研究的严谨性和可靠性。财务数据分析提供可靠数据底座，财报审阅与企业深度研究夯实个股认知，行业分析把握产业趋势，投资组合审查实现策略落地与风险控制，每日简报提供实时监控，组合调仓执行策略转换。通过强制性的正文验证、统一的工作流执行标准和完善的门禁机制，可实现高效、可复现、可扩展的研究流水线。
