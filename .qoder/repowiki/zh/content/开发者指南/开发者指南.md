...
## 更新摘要
**所做更改**
- 新增Qoder平台集成支持，通过根级package.json增强插件发现能力
- 添加多平台技能同步机制，支持Claude Code、Codex、TRAE、DSH、ZCode和Qoder的统一工作流
- 增强插件架构和生态系统内的可发现性，实现跨平台的技能共享
- 更新工作流清理和会话状态管理功能，确保工作流状态一致性
- 强化持久化机构记忆能力，通过结构化目录组织学习知识
...
## 简介
本指南面向希望参与本项目开发与贡献的工程师与研究者，提供从环境搭建、协作规范到技术扩展、测试策略与安全实践的全流程说明。项目围绕投研工作流构建，包含技能（Skills）、提示词（Prompts）、工具脚本与数据资产等模块，支持通过配置化方式扩展新能力与集成外部API。**最新更新**：引入了Qoder平台集成支持，通过增强的插件发现机制和多平台技能同步，显著提升了生态系统的可扩展性和跨平台兼容性。同时集成了学习知识库系统和 SessionStart 钩子机制，实现了跨会话的机构记忆和学习沉淀功能。
...
- **新增**：Qoder平台集成：`.qoder/plugins/`（项目级插件目录）
- **新增**：多平台技能同步：scripts下的各平台同步脚本
...
A --> G["Qoder平台集成<br/>.qoder/plugins/"]
A --> H["多平台同步脚本<br/>sync-*-skills.py"]
A --> I["学习知识库<br/>.learnings/"]
A --> J["钩子配置<br/>.claude/settings.json"]
```
...
- **新增**：Qoder平台集成：通过 `.qoder/plugins/` 目录实现项目级插件发现和集成
- **新增**：多平台同步机制：统一的技能源（skills/*.md）自动同步到各个平台
...
## 架构总览
系统由"技能编排 + 提示词驱动 + 工具执行 + 数据支撑 + 学习记忆 + 多平台集成"构成。技能作为高层任务描述，调用相应工具完成数据获取、处理与输出；提示词提供模型交互模板；工具负责具体计算与I/O；数据层提供输入与中间结果；**学习系统**通过 SessionStart 钩子在每个会话开始时自动注入累积的学习知识，实现跨会话的机构记忆；**多平台集成**通过统一的技能源和同步脚本，将技能分发到不同的AI开发平台。
...
subgraph "多平台集成"
M1["Qoder平台<br/>.qoder/plugins/"]
M2["Codex平台<br/>codex-skills/"]
M3["TRAE平台<br/>.trae/skills/"]
M4["DSH平台<br/>.dsh/skills/"]
M5["ZCode平台<br/>.zcode/skills/"]
MS["统一技能源<br/>skills/*.md"]
end
subgraph "学习系统"
...
- **新增**：Claude Code 钩子配置
  ...
  - 验证 `jq` 命令可用（用于学习摘要生成）。
- **新增**：Qoder平台配置
  - 确保 `.qoder/settings.json` 正确配置插件发现路径。
  - 运行 `python3 scripts/sync-qoder-skills.py` 生成项目级插件。
...
- **新增**：多平台技能贡献
  - 修改统一技能源 `skills/*.md`，运行相应的同步脚本生成各平台版本。
  - 使用 `--check` 参数验证同步结果的一致性。
...
### 技术架构与扩展点
- 新技能开发
  - 在skills目录下新增SKILL.md定义任务目标、输入输出与步骤。
...
- **新增**：Qoder平台集成
  - 通过 `scripts/sync-qoder-skills.py` 将统一技能源转换为Qoder项目级插件。
  - 插件位于 `.qoder/plugins/ai-berkshire-investment/skills/` 目录。
  - Qoder通过 `.qoder/plugins/<plugin>/skills/` 路径发现项目级插件。
- **新增**：多平台同步机制
  - 统一的技能源（skills/*.md）被同步到多个AI开发平台。
  - 每个平台有特定的适配器和元数据格式。
  - 支持检查模式（`--check`）验证同步结果的一致性。
- **新增**：学习系统集成
...
participant Skill as "统一技能源<br/>skills/*.md"
participant Sync as "同步脚本<br/>sync-*-skills.py"
participant Platforms as "多平台输出<br/>各平台目录"
...
Dev->>Skill : 编写/更新统一技能
Note over Sync : 运行同步脚本
Sync->>Platforms : 生成各平台版本
...
### 多平台技能同步机制
**新增功能**：多平台技能同步机制实现了统一技能源到多个AI开发平台的自动分发。

- 支持的平台
  - Qoder：通过 `.qoder/plugins/ai-berkshire-investment/skills/` 发现项目级插件
  - Codex：通过 `codex-skills/` 目录分发技能
  - TRAE：通过 `.trae/skills/` 目录分发技能
  - DSH：通过 `.dsh/skills/` 目录分发技能
  - ZCode：通过 `.zcode/skills/` 目录分发技能

- 同步脚本功能
  - 从统一技能源（`skills/*.md`）读取原始技能定义
  - 为每个平台生成特定的元数据和适配器说明
  - 支持检查模式（`--check`）验证同步结果的一致性
  - 自动生成平台特定的中文描述和调用指导
...
```mermaid
flowchart TD
Start(["统一技能源<br/>skills/*.md"]) --> Qoder["Qoder同步脚本<br/>sync-qoder-skills.py"]
Start --> Codex["Codex同步脚本<br/>sync-codex-skills.py"]
Start --> Trae["TRAE同步脚本<br/>sync-trae-skills.py"]
Start --> DSH["DSH同步脚本<br/>sync-dsh-skills.py"]
Start --> ZCode["ZCode同步脚本<br/>sync-zcode-skills.py"]
Qoder --> QoderOut[".qoder/plugins/*/skills/"]
Codex --> CodexOut["codex-skills/"]
Trae --> TraeOut[".trae/skills/"]
DSH --> DSHOut[".dsh/skills/"]
ZCode --> ZCodeOut[".zcode/skills/"]
```
...
- **新增**：多平台同步脚本
  - sync-qoder-skills.py：生成Qoder项目级插件
  - sync-trae-skills.py：生成TRAE项目级技能
  - sync-dsh-skills.py：生成DSH项目级技能
  - sync-zcode-skills.py：生成ZCode工作区级技能
...
Choose --> MultiPlatform["多平台同步脚本"]
InstallSkills --> SyncSkills["同步技能"]
InstallPrompts --> SyncPrompts["同步提示词"]
MultiPlatform --> SyncQoder["同步到Qoder"]
MultiPlatform --> SyncCodex["同步到Codex"]
MultiPlatform --> SyncTrae["同步到TRAE"]
MultiPlatform --> SyncDSH["同步到DSH"]
MultiPlatform --> SyncZCode["同步到ZCode"]
SyncSkills --> End(["结束"])
SyncPrompts --> End
SyncQoder --> End
SyncCodex --> End
...
- 内部依赖
  ...
  - **新增**：多平台同步机制依赖于统一技能源和各平台特定的元数据格式。
...
```mermaid
graph LR
Skills["统一技能源<br/>skills/*.md"] --> Sync["多平台同步脚本"]
Sync --> Platforms["各平台输出"]
Platforms --> Tools["工具脚本"]
Tools --> Data["数据层"]
Tools --> API["外部API/数据源"]
Hooks["钩子系统"] --> Skills
Hooks --> Tools
...
- 常见问题
  ...
  - **新增**：多平台同步问题
    - 同步脚本执行失败：检查Python环境和依赖库。
    - 平台特定错误：查看各平台输出的适配器说明。
    - 检查模式失败：使用 `--check` 参数验证同步结果。
  - **新增**：学习系统问题
...
## 结论
本项目以技能与提示词为核心，结合工具链与数据资产，形成可扩展的投研工作流。**最新更新**：通过引入Qoder平台集成支持和多平台技能同步机制，显著提升了生态系统的可扩展性和跨平台兼容性。通过学习知识库系统和 SessionStart 钩子机制，实现了跨会话的机构记忆和学习沉淀功能。通过清晰的贡献规范、完善的模板与脚本，以及新增的多平台集成和学习系统，开发者可以快速上手并持续演进系统能力。建议在新功能开发中遵循模块化设计、强化测试与安全实践，以提升整体质量与稳定性。
