...
- **加密市场温度判定（v5.5 修订，必须）**：按 `config/portfolio-targets.md` v5.5 "加密市场温度动态规则"执行：
  - 搜索 "bitcoin price all time high distance"、"crypto fear greed index"、"bitcoin ETF flows"、"crypto regulation stablecoin" → 确认加密水位（🟢积累/🟡复苏/**🟡+复苏加速**/🟠狂热/🔴崩盘）
  - **🟡+ 判定（v5.5 新增，必查）**：F&G 60-85 且自 <30 回升 + BTC 底部回升 20-60% + ETF 月净流入 >$2B + 空头清算 >$1B + 事件催化 <30 天——满足 ≥3 条即判 🟡+（配比 standard 12-18%/aggressive 18-30%）。**9/2 教训**：v5.4 无此带导致复苏-狂热过渡段（CRCL 20 日 +57%）被误判为"超上限必须减仓"
  - 输出：加密水位 + 风险偏好档位（standard/aggressive，**每次执行开头必须向用户确认或沿用上次声明**）+ 当前应配比例 + 30 日动量数据行（BTC 与各加密持仓）+ 内部路径选择，写入报告头部
- **偏差裁决升级条款（v5.5，优先级高于偏离修正规则）**：实际加密暴露 > 温度带上限 且 该仓位 30 日动量 >+20% → 不自动触发🔴强制减仓，报告原话呈现三选项（①按带修正②切换 aggressive 档③维持现状接受标注）交用户裁决；用户裁决后 48h 内不重复追问，daily-brief 保留一行标注
