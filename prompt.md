...
>
> **⚠️ v6.0 迁移注记（2026-08-25 用户指令）**：本文件的 v4 目标分布（五类静态比例、持仓≤11只）已被 `config/portfolio-targets.md` v6.0 **三大主仓结构取代**：加密 ≤40% / AI平台或软件 ≤30% / AI硬件 ≤30%（硬上限），实际配置由**大方向热度分**（每方向 5 项信号实时打分 0-100）连续映射动态计算，方向内部按**子方向机会承接**（机会分=估值40+催化30+逻辑30，准入双条件：机会分≥50 且子方向热度<75）决定钱去哪，允许全押单一子方向。**一切动态计算，禁止写死比例。** 本文件第 38-46 行目标分布、第 13 行持仓≤11 只等内容作废，仅作设计演进存档；执行一律以 config/portfolio-targets.md v6.0 为准。
...
**AI总暴露**：50-75%（平台+软件+硬件+修复+爆发之和），硬上限80%（v5.5，随 AI 整体水位动态）

**约束分级（机会优先，风险靠硬上限和止损管；区间数值以 config/portfolio-targets.md 为准）**：
- **硬约束（🔴强制修正）**：AI总暴露 50-75%/cap 80、非AI对冲 10-15%/cap 20、现金动态带（温度判定，见config"现金动态规则"）、**单一非美国家敞口≤15%**（硬上限20%，含中概ADR/港股——地缘/退市/汇率尾部风险）
...
3. **偏离修正规则（v4：只适用于硬约束类别——AI总暴露/非AI价值/现金/单一国家敞口）**：
   ...
   - **硬上限触发**（如AI总暴露>80%、单一非美国家>20%）：必须在下一轮调仓时减仓至上限以下，不可例外
...
| *AI总暴露（小计·不参与求和）* | 50-75% | 80% | XX% | XX% | ±X% | ✅/⚠️/🔴 |
