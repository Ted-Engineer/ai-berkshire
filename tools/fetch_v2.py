#!/usr/bin/env python
"""Stock data fetcher using yfinance with SSL workaround."""
import sys, json, time, os
# Disable SSL verification entirely
os.environ['PYTHONHTTPSVERIFY'] = '0'
import yfinance as yf
import urllib3
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

def fetch(tickers):
    results = {}
    for tk in tickers:
        try:
            t = yf.Ticker(tk)
            # Force use of requests session with verify=False
            import requests
            s = requests.Session()
            s.verify = False
            t.session = s
            i = t.info
            results[tk] = {
                'price': i.get('currentPrice') or i.get('regularMarketPrice'),
                'market_cap': i.get('marketCap'),
                'trailing_pe': i.get('trailingPE'),
                'forward_pe': i.get('forwardPE'),
                'ps': i.get('priceToSalesTrailing12Months'),
                'pb': i.get('priceToBook'),
                'revenue': i.get('totalRevenue'),
                'fcf': i.get('freeCashflow'),
                'op_margin': i.get('operatingMargins'),
                'profit_margin': i.get('profitMargins'),
                'roe': i.get('returnOnEquity'),
                'cash': i.get('totalCash'),
                'debt': i.get('totalDebt'),
                'rev_growth': i.get('revenueGrowth'),
                'eps_growth': i.get('earningsGrowth'),
                '52w_high': i.get('fiftyTwoWeekHigh'),
                '52w_low': i.get('fiftyTwoWeekLow'),
                'dividend_yield': i.get('dividendYield'),
                'shares': i.get('sharesOutstanding'),
                'gross_margin': i.get('grossMargins'),
                'name': i.get('longName') or i.get('shortName'),
            }
            print(f"  OK {tk}: ${i.get('currentPrice') or i.get('regularMarketPrice')} fwdPE={i.get('forwardPE')}", file=sys.stderr)
        except Exception as e:
            results[tk] = {'error': str(e)[:200]}
            print(f"  ERR {tk}: {str(e)[:100]}", file=sys.stderr)
        time.sleep(0.3)
    return results

if __name__ == '__main__':
    tickers = sys.argv[1].split(',')
    r = fetch(tickers)
    print(json.dumps(r, indent=2, default=str))
